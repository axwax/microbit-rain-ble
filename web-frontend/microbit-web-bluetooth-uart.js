...
function readTx(event) {
    ...
    if (!("TextDecoder") in window) {
        ...
        let ts = new Date().toISOString().replace(/[^0-9/-]/gm, "").substr(0,14);
        document.getElementById("tx").innerHTML += ts + " " + enc.decode(event.target.value) + "<br>";
        //console.log(enc.decode(event.target.value).substr(5));
        let rainVal = parseFloat(enc.decode(event.target.value).substr(5));
        let body = 'rain rain='+rainVal.toString();
        console.log(body);
        fetch('http://192.168.0.100:8086/write?db=mydb',{method:'POST', body: body});
        //fetch('http://192.168.0.100:8086/write?db=mydb',{'method':'POST', 'body': body});
